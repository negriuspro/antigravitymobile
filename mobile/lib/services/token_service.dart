...
const kQuotas = [
  ProviderQuota(
    ...
    color: '#F55036',
    specialty:
        'Respuestas ultrarrápidas — prototipos, chat en tiempo real, testing veloz',
    resetCycle: 'diario',
  ),
  ProviderQuota(
    id: 'cerebras',
    ...
    color: '#7D3C98',
    specialty:
        'Velocidad extrema con modelos grandes — GPT-OSS 120B a velocidad increíble',
    resetCycle: 'diario',
  ),
  ProviderQuota(
    id: 'gemini',
    ...
    color: '#4285F4',
    specialty:
        'Imágenes y contexto largo — análisis visual, documentos de 1M tokens, multimodal',
    resetCycle: 'diario',
  ),
  ProviderQuota(
    id: 'openrouter',
    ...
    color: '#10B981',
    specialty:
        'Acceso a 300+ modelos — elige el mejor para cada tarea, incluye modelos gratuitos',
    resetCycle: 'diario',
  ),
  ProviderQuota(
    id: 'claude_code',
    ...
    color: '#CC785C',
    specialty:
        'Código complejo — debugging, arquitectura, refactoring, lectura de archivos del proyecto',
    resetCycle: 'mensual',
  ),
  ProviderQuota(
    id: 'sambanova',
    ...
    color: '#06B6D4',
    specialty:
        'Modelos grandes en hardware dedicado — análisis de datos, transformaciones, razonamiento',
    resetCycle: 'diario',
  ),
];
...
class TokenUsage {
  ...
  TokenUsage copyWith(
          {int? requestsToday, int? tokensToday, DateTime? lastReset}) =>
      TokenUsage(
        providerId: providerId,
...
class TokenService {
  ...
  Future<Map<String, TokenUsage>> loadAll() async {
    ...
    return map.map(
        (k, v) => MapEntry(k, TokenUsage.fromJson(v as Map<String, dynamic>)));
  }

  Future<void> recordRequest(String providerId, {int tokens = 0}) async {
    final all = await loadAll();
    final now = DateTime.now().toUtc();
    var usage = all[providerId] ??
        TokenUsage(
            providerId: providerId,
            requestsToday: 0,
            tokensToday: 0,
    ...
    if (now.day != usage.lastReset.day ||
        now.difference(usage.lastReset).inHours >= 24) {
      usage = TokenUsage(
          providerId: providerId,
          requestsToday: 0,
          tokensToday: 0,
    ...
    usage = usage.copyWith(
        ...
        tokensToday: usage.tokensToday + tokens);
